\new{
Decentralized networks can rely on a honest party - the coordinator node - to verify that the allocation properties are respected. If the coordinator node notices that a node is offline or failing to prove the possession of a resource slice, it can reassign the slice to another node, so that the security and availability guarantees are still enforced.
