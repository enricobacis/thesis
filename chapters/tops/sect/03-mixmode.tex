...
\begin{figure*}[!t]
	...
	\caption{\label{ms:fig:oaep-issue}Classical OAEP does not evenly mix the plaintext}
\end{figure*}

\begin{figure*}[!t]
	\centering
	{\includegraphics[width=.85\columnwidth]{figures/oaep-schema}}
	\caption{\label{ms:fig:oaep}\name algorithm based on OAEP}
\end{figure*}

Another approach that provides strong inter-dependency in the representation of a resource is {\em Optimal Asymmetric Encryption Padding} (OAEP) \cite{bellare1995optimal, bellare1998relations} proposed by Bellare and Rogaway. OAEP is a form of Feistel network \cite{feistel1973cryptography} that uses a pair of cryptographically one-way trapdoor functions, $G$ and $H$, to process the plaintext prior to asymmetric encryption. The OAEP process is represented in Figure~\ref{ms:fig:oaep-issue}.
