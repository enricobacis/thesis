...
The OAEP-based implementation is faster than the AES one that does not leverage the hardware implementation. In fact, even tough the base application of $AES$ and  $SHA2_{512}$ share a similar performance profile, the OAEP approach has a benefit due to the reduction on the number of rounds due to larger mini-blocks. On architectures that provide a native hardware implementation for AES, its use shows the best performance profile for \name and should be preferred.
