...
%
\new{
	In this chapter we present a solution that combines an All-or-Nothing
...
\new{  
\subsection{A note on DCS dynamicity}
...
%
In \cite{globecom}
we have studied how to combine the method presented in this Chapter with Fountain Codes~\cite{fountain-codes,luby2011raptorq,shokrollahi2006raptor} in order to better exploit the dynamicity of decentralized cloud storage networks.
...
Although probabilistic, fountain codes have two main characteristics that allow us to profitably use them in the DCS context. First, they are  {\em rateless}, that is, using these codes it is possible to create a new (i.e., different from each other) shard on the fly and therefore the number $n$ of encoded shards is not fixed a priori.
...
In DCS systems we can rely on the coordinator node to verify that the state is compliant with the thresholds. This can be achieved by offloading Proof-of-Retrievability~\cite{bowers2009proofs} challenges and responses to the coordinator node. The challenges will then be issued to the storage nodes, and their responses compared with the expected ones. In case a response do not match, the coordinator will select another storage node and move the slice accordingly.
%
